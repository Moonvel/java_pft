...
public class SoapHelper {
    ...
    public Set<Project> getProjects() throws MalformedURLException, ServiceException, RemoteException {
        ...
        return Arrays.asList(projects).stream().map(
                (p) -> new Project().withId(p.getId()).withName(p.getName())).collect(Collectors.toSet());
    }
    ...
    public Issue addIssue(Issue issue) throws MalformedURLException, ServiceException, RemoteException {
        ...
        issueData.setPriority(new ObjectRef(issue.getProject().getId(), issue.getProject().getName()));
        ...
        return new Issue().withId(createdIssueData.getId().intValue()).withSummary(createdIssueData.getSummary()).
                withDescription(createdIssueData.getDescription()).withProject(new Project().withId(BigInteger.valueOf(createdIssueData.getProject().getId().intValue())).
